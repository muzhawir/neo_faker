...
- Added `NeoFaker.seed/1` — seeds `:rand` for the calling process, for reproducible output in
...
- **`NeoFaker.Person.first_name/1`, `middle_name/1`, `last_name/1`, `full_name/1`,
  ...
  one — e.g. `NeoFaker.set_locale(:id_id)` followed by `NeoFaker.Person.first_name()` still
