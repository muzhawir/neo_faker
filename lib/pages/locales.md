...
NeoFaker supports locale-specific data for realistic test output. Locale-aware functions —
almost every function that accepts a `:locale` option — load their data from
...
`:default` is not itself a locale code — it's the baseline dataset every other locale falls back
to. `:en_us` is accepted as a locale value but currently has no dataset of its own; it resolves
...
mirroring the domains it covers (only the files you provide are used — anything missing falls
