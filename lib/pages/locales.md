...
functions use data files from `priv/locales/<locale>/`. If a requested locale is missing, NeoFaker falls back to the default (`:default`), which provides generic English (US) data.
...
To set the locale, use the `locale` option in the function call. See [configuration instructions](getting-started.html#configuration) for details.
